...
class TicketMessageController extends Controller
{
    ...
    public function store(Request $request, Ticket $ticket)
    {
        ...
        // Manager-loop fallback: when the raiser replies on a ticket with no
        // assigned PIC, the recipient set would otherwise be empty (raiser
        // can't notify themselves). Bring the dept's manager pool in so the
        // team sees the reply and can pick the ticket up as PIC.
        ...
        if ($recipientIds->isNotEmpty()) {
            ...
            // Email after the HTTP response is returned. Mirrors the proven
            // AnnouncementController pattern. Uses Mail::send() (not queue())
            // because terminating() already moves the work out of the request
            // path. Pass the resolved recipient ids in via use() so the email
            // dispatch matches the bell exactly.
            $ticketId = $ticket->id;
    ...
    /**
     * Serialise a message for the chat JSON feed. Normalises legacy
     * single-attachment columns AND new multi-attachment table into one
     * `attachments` array.
     */
