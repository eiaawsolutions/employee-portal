...
class BirthdayWishMail extends Mailable
{
    ...
    /**
     * Resolve the public URL for the employee's company logo. Returns null
     * when no logo is on file so the template can fall back gracefully.
     * Company is tenant-scoped via BelongsToTenant — the lookup automatically
     * stays inside the current tenant.
     */
    private function resolveCompanyLogoUrl(): ?string
    {
        if (empty($this->employee->company)) {
            return null;
        }
