...
<div class="card" style="max-width:760px;">
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('tickets.store') }}" enctype="multipart/form-data" id="ticketCreateForm">
            ...
            {{-- Hidden routing field, set by JS: the dept that should handle
                 the ticket (resolved from the chosen subject). Multi-company
                 cluster routing is intentionally absent — within a tenant
                 every dept handles every ticket in its area. --}}
            ...
            <div class="ticket-routing">
                ...
                {{-- Manual department picker — shown when the user picks
                     "Other" and keyword inference can't decide the dept. --}}
...
(function () {
    ...
    function syncSubmitEnabled() {
        var ok = !!deptHidden.value;
        submitBtn.disabled = !ok;
        submitBtn.title = ok ? '' : 'Pick a subject so we can route the ticket.';
    }
    ...
    // Client-side keyword inference for "Other" subjects.
    ...
    function handleSubjectChange() {
        ...
        if (isOther) {
            ...
            if (inferred) {
                setRouting(inferred, 'auto');
                overrideWrap.style.display = 'none';
            } else if (subjectOther.value.trim() !== '') {
                setRouting(null, 'needs-dept');
    ...
    // Manual dept picker (shown when "Other" + ambiguous keyword inference).
    ...
    // If a validation redirect restored a manual department pick, honour it.
    if (oldDepartment && deptHidden.value !== oldDepartment) {
        setRouting(oldDepartment, 'manual');
        overridePicker.value = oldDepartment;
        overrideWrap.style.display = 'block';
    }
    ...
    if (ticketForm && submitBtn) {
        ticketForm.addEventListener('submit', function (e) {
            if (!deptHidden.value) {
                ...
                routingDept.textContent = 'Please pick a subject first';
                return;
            }
